...
echo "===== OPPO/OnePlus/realme SM8550 universal 6.1.57 A14 OKI kernel local build script by Coolapk@cctv18 ====="
...
cd kernel_workspace
git clone --depth=1 https://github.com/cctv18/android_kernel_common_oneplus_sm8550 -b oneplus/sm8550_u_14.0.0_oneplus11 common
...
if [[ "$KSU_BRANCH" == "y" ]]; then
  ...
  # Temporary fix for task_mmu.c missing show_pad after applying patches on some kernels
...
if [[ "$APPLY_LZ4KD" == "y" || "$APPLY_LZ4KD" == "Y" ]]; then
  wget https://raw.githubusercontent.com/cctv18/oppo_oplus_realme_sm8550/refs/heads/main/other_patch/zram.zip
fi
